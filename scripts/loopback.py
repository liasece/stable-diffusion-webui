...
class Script(scripts.Script):
    ...
    def run(self, p, loops, final_denoising_strength, denoising_curve, append_interrogation):
        ...
        if len(history) > 1:
            ...
            if opts.grid_save:
                images.async_save_image(grid, p.outpath_grids, "grid", initial_seed, p.prompt, opts.grid_format, info=info, short_filename=not opts.grid_extended_filename, grid=True, p=p)
